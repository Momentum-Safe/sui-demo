...
/// Forked from sui-framework, and add function to access it'e length
module demo_msafe::priority_queue {
    use std::vector;
    ...
    public fun is_empty<T: drop>(pq: &PriorityQueue<T>) : bool {
        vector::is_empty(&pq.entries)
    }

    /// borrow the entry with the highest priority value.
    public fun borrow_max<T: drop+copy>(pq: &PriorityQueue<T>) : (u64, T) {
        let len = vector::length(&pq.entries);
        assert!(len > 0, EPopFromEmptyHeap);
        let entry = vector::borrow(&pq.entries, 0);
